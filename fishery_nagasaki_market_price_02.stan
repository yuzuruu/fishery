...
model {
  ...
  // 観測空間
  for(i in 1:n_pos){
    y[i] ~ normal(x[col_indx_pos[i], row_indx_pos[i]] + season[col_indx_pos[i], row_indx_pos[i]], sd_r[row_indx_pos[i]]);
  }
}
// 尤度計算と推定。
generated quantities {
  ...
  // 尤度計算
  for (n in 1:n_pos) log_lik[n] = normal_lpdf(y[n] | x[col_indx_pos[n], row_indx_pos[n]] + season[col_indx_pos[n], row_indx_pos[n]], sd_r[row_indx_pos[n]]);
  // yを推定する
  for(i in 1:N){
    for(t in 1:TT){
      yhat[t,i] = normal_rng(x[t,i] + season[t,i], sd_r[i]);
      }
      }
    }
